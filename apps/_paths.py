
def get_paths():
    csv_paths = [
                'csv/combined_news.csv',
                'csv/ref_industry_news.csv',
                'csv/cooling_post_news.csv',
                'csv/natural_refrigerants_news.csv',
                'csv/trane_technologies_news.csv',
                'csv/danfoss_news.csv',
                'csv/LG_News.csv',
                'csv/copeland_news.csv',
                'csv/carrier_news.csv',
                'csv/viessman_news.csv',
                'csv/lennox_news.csv',
                'csv/BDR_Thermea_news.csv',
                'csv/honeywell_news.csv',
                # 'csv/trane_commercial_news.csv',
                'csv/thermoking_news.csv',
                'csv/thermoking_EU_news.csv',
                'csv/carel_news.csv',
                'csv/deltatrak_news.csv',
                'csv/bitzer_news.csv',
                'csv/jarn_news.csv',
                'csv/ehpa_news.csv',
                'csv/contracting_business_news.csv',
                'csv/contractor_mag_news.csv',
                'csv/achr_news.csv',
                'csv/acr_journal_news.csv',
                'csv/fleet_owner_news.csv',
                'csv/rac_plus_news.csv',
                'csv/phcp_pros_news.csv',
                'csv/HV_UK_news.csv',
                'csv/climate_control_news.csv',
                'csv/DOE_news.csv',
                'csv/iea_news.csv',
                'csv/hpa_news.csv',
                'csv/LG_HVAC_NA_news.csv',
                'csv/LG_Electronics_NA.csv',
                'csv/generac_news.csv',
                'csv/nidec_news.csv',
                'csv/beijer_ref_news.csv',
                'csv/daikin_corporate_news.csv',
                'csv/daikin_europe_news.csv',
                'csv/daikin_applied_news.csv',
                'csv/GEA_news.csv',
                'csv/stiebel_eltron_news.csv',
                'csv/resideo_news.csv',
                'csv/resideo_investor_news.csv',
                'csv/spx_news.csv',
                'csv/rheem_news.csv',
                'csv/hussmann_news.csv',
                'csv/panasonic_hvac_news.csv',
                'csv/panasonic_global_news.csv',
                'csv/panasonic_uk_news.csv',
                'csv/atmoszero_news.csv',
                'csv/mesa_labs_news.csv',
                'csv/mitsubishi_electric_global_news.csv',
                'csv/mitsubishi_electric_hvac_news.csv',
                'csv/mitsubishi_electric_sci_news.csv',
                'csv/parker_sporlan_news.csv',
                'csv/rechi_news.csv',
                'csv/schott_news.csv',
                'csv/secop_news.csv',
                'csv/sensata_news.csv',
                'csv/tive_news.csv',
                'csv/nibe_news.csv',
                'csv/tecumseh_news.csv',
                'csv/watsco_news.csv',
                'csv/sanhua_news.csv',
                'csv/sanhua_group_news.csv',
                'csv/icm_controls_news.csv',
                'csv/midea_news.csv',
                'csv/rees_scientific_news.csv',
                'csv/carrier_group_news.csv',
                'csv/failed_scrapers.csv',
                'csv/ecobee_news.csv',
                'csv/johnson_controls_news.csv',
                'csv/hitachi_news.csv'
            ]
    return csv_paths